...
export const FacultyHistoryPage: React.FC = () => {
  ...
  const columns = [
    ...
    {
      header: 'Student Name',
      cell: (row: HistoryItem) => <span className="font-semibold text-slate-900">{row.student_name}</span>,
    },
    {
      header: 'Course & Section',
      cell: (row: HistoryItem) => (
        <div>
          <span className="font-bold text-slate-800 text-xs">{row.class_code}</span>
          <span className="text-[11px] text-slate-500 font-medium block">{row.section || 'Sec 1'} • {row.room || 'Lab 1'}</span>
        </div>
      ),
    },
    {
    ...
    {
      header: 'Notes',
      cell: (row: HistoryItem) => <span className="text-[11px] text-slate-500 font-medium">{row.notes || 'Verified'}</span>,
    },
  ];
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Attendance History Logs</h1>
          ...
          <p className="text-xs text-slate-500 mt-0.5">View, filter, and export student attendance logs across all classes</p>
        </div>

        <Button variant="primary" onClick={handleExportCSV}>
          <Download className="w-4 h-4 mr-1.5" /> Export to CSV
        </Button>
      </div>

      {/* Filter Card */}
      <Card title="Filter History Records" subtitle="Filter attendance history by Date, Class, and Status">
      ...
      <Card title="Attendance Records Table" subtitle={`Displaying ${records.length} records`}>
        {isLoading ? (
          <div className="py-8 text-center text-xs text-slate-400">Loading attendance history...</div>
        ) : (
